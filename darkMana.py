...
SWARM_MANAGER_CONF_DEFAULTS = {
    'pid_file': '{xdg_data_home}/DSreal_swarm_manager.pid',
    'log_file': '{xdg_data_home}/DSreal_swarm_manager.log',
...
class SwarmManager:
    ...
    def _maintain_pool(self):
        ...
        for constraints, needed_dict in self._state.slots_needed(waiting, active).items():
            ...
            if slots_needed > 0:
                self.NodesSpan(constraints, services, slots_needed)
            elif slots_needed < 0:
    ...
    def _terminate_if_idle(self):
        ...
        for node in self._docker_interface.nodes():
            if node.task_count > 0:
                return  # nodes are running a DSreal service
    ...
    def GetSpawnProp(self, constraints, constraint_name, services):
        if services:
    ...
    def NodesSpan(self, constraints, services, slots_needed):
        ...
        command = self._conf.spawn_command.format(
            ...
            image=self.GetSpawnProp(constraints, IMAGE_CONSTRAINT, services) or '',
            cpus=self.GetSpawnProp(constraints, CPUS_CONSTRAINT, services) or '',
            slots=slots_needed,
        )
...
def _configure_logging(args, conf):
    ...
    log = logging.getLogger(__name__)
    gxlog = logging.getLogger('DSreal')
