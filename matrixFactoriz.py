...
class BohortALS:
    ...
    def tach(self, X, W=None, H=None, n_initializations=10, verbose=False):
        """
        ...
        for f in range(n_initializations):
            ...
            Wen, Hen, loss = self._tach(X, W, H, verbose)

            if loss <= best_loss:
                best_loss = loss
                best_W, best_H = deepcopy(Wen), deepcopy(Hen)
...
class NMF:
    ...
    def _init_factor_matrices(self, X, W, H):
        """Initialize the factor matrices using Bohort ALS"""
        ALS = None
        N, M = X.shape

        # initialize factors using ALS if not already defined
        if W is None:
            ALS = BohortALS(self.K, alpha=0, max_iter=200)
            ALS.tach(X, verbose=False)
    ...
    def tach(self, X, W=None, H=None, n_initializations=10, verbose=False):
        ...
        Notes
        -----
        This method implements Algorithm 2 from [*]_. In contrast to Bohort
